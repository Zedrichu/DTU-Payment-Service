...
public class AccountStepDefs {
	...
	@When("a {string} event for a merchant is received with missing bank account")
	public void aEventForAMerchantIsReceivedWithMissingBankAccount(String arg0) {
		eventTypeName = EventTypes.fromTopic(arg0);
		merchantNoBank = new Merchant("test", "test", "123136-1243", "", null);
		correlator = Correlator.random();
		accountManagementService.handleMerchantRegistrationRequested(new Event(eventTypeName.getTopic(), new Object[] { merchantNoBank, correlator }));
	}

	@Then("the {string} event is sent with same correlation id")
	public void theEventIsSentWithSameCorrelationId(String eventName) {
		eventTypeName = EventTypes.fromTopic(eventName);
		if (eventName.contains("Failure") || eventName.contains("Failed")) {
			badEventCaptor = ArgumentCaptor.forClass(Event.class);
			verify(queue).publish(badEventCaptor.capture());
	...
	@And("the merchant receives a failure message {string}")
	public void theMerchantReceivesAFailureMessage(String arg0) {
		assertEquals(arg0, receivedEvent.getArgument(0, String.class));
	}
}
